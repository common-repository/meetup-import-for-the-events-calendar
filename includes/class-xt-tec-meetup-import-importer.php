 ...
 */
class XT_TEC_Meetup_Import_Importer {
	...
	public function xtmi_load_importer() {
		// Import on init.
		add_action( 'xtmi_run_import', array( $this, 'xtmi_run_importer' ), 100 );
	}
		...
		if ( ! is_wp_error( $meetup_response ) ) {
			...
			if ( is_array( $meetup_events ) && ! isset( $meetup_events['errors'] ) ) {
				foreach ( $meetup_events as $meetup_event ) {
					$this->xtmi_save_event( $meetup_event, $meetup_group_id, $post_id );
				}
			}
		}
	}
	 ...
	 * @return void
	 */
	public function xtmi_save_event( $meetup_event = array(), $meetup_group_id = '', $post_id = null ) {
