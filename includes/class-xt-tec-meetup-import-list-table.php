 ...
 */
class XT_TEC_Meetup_Import_List_Table extends WP_List_Table {
	...
	function column_title( $item ) {
		...
		// Build row actions.
		$actions = array(
		    'delete' => sprintf( '<a href="%1$s" onclick="return confirm(\'Warning!! Are you sure to Delete this Meetup url? Url will be permanatly deleted.\')">%2$s</a>',esc_url( wp_nonce_url( add_query_arg( $xtmi_url_delete_args ), 'xtmi_delete_url_nonce' ) ), esc_html__( 'Delete', 'xt-tec-meetup-import' ) ),
		);
	...
	function column_action( $item ) {
		...
		return sprintf( '<a class="button-primary" href="%1$s">%2$s</a>',
			...
			esc_html__( 'Import Now', 'xt-tec-meetup-import' )
		);
	}
