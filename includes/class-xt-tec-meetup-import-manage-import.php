 ...
 */
class XT_TEC_Meetup_Import_Manage_Import {
	...
	public function __construct( $plugin_name, $version ) {
		...
		add_action( 'init', array( $this, 'xtmi_handle_insert_form' ), 100 );
		add_action( 'init', array( $this, 'xtmi_handle_delete_url' ), 100 );
		...
		add_action( 'admin_notices', array( $this, 'xtmi_display_success_message' ), 100 );
	 ...
	 * @since    1.0.0
	 */
	public function xtmi_handle_insert_form() {
		if ( isset( $_POST['xtmi_action'] ) && $_POST['xtmi_action'] == 'xtmi_url_submit' &&  check_admin_referer( 'xtmi_insert_form_nonce_action', 'xtmi_insert_form_nonce' ) ) {
			...
			if ( ! empty( $xtmi_url ) && empty( $this->errors ) ) {
				$this->xtmi_save_meetup_url( $xtmi_url, $xtmi_cats );
			}
		}
	}
	 ...
	 * @since    1.0.0
	 */
	public function xtmi_handle_delete_url() {
	...
	public function xtmi_handle_manual_import() {
		if ( isset( $_GET['xtmi_action'] ) && $_GET['xtmi_action'] == 'xtmi_run_import' && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'xtmi_run_import_nonce') ) {
			$url_id = absint( $_GET['url_id'] );
			if ( $url_id > 0 ) {
				do_action( 'xtmi_run_import', $url_id );
				$message = esc_html__( 'Event imported successfully.', 'xt-tec-meetup-import' );
	...
	public function xtmi_format_url( $url ) {
		...
		if ( empty( $url ) ) {
			$this->errors[] = __('meetup group url is empty please provide meetup group url.',  'xt-tec-meetup-import');
			return '';
		}
		// check provide url is valid.
		if ( filter_var( $url, FILTER_VALIDATE_URL ) === false ) {
		    $this->errors[] = __( 'Not valid meetup group url please provide valid meetup group url.',  'xt-tec-meetup-import');
			return '';
		}
	 ...
	 */
	public function xtmi_save_meetup_url( $url, $cats = array() ) {

		if ( $this->xtmi_post_exists( $url ) > 0 ) {
			$this->errors[] = __( 'Meetup group url already exists', 'xt-tec-meetup-import');
			return false;
		}

		$insert_args = array(
			'post_title' => $url,
			'post_type' => 'xtmi_meetup_url',
			'post_status' => 'publish',
		);
		...
		if ( is_wp_error( $insert ) ) {
			$this->errors[] = __('Something went wrong when insert url. ',  'xt-tec-meetup-import') . $insert->get_error_message();
			return;
		}
	 ...
	 * @return boolean
	 */
	public function xtmi_post_exists( $url ) {
		global $wpdb;
		$post_title = wp_unslash( sanitize_post_field( 'post_title', $url, 0, 'db' ) );

		$query = "SELECT ID FROM $wpdb->posts WHERE `post_type`= 'xtmi_meetup_url'";
		$args = array();
	...
	public function xtmi_display_insert_success() {
	    ...
	    <div class="notice notice-success is-dismissible">
	        <p><?php _e( 'Meetup URL for import inserted successfully.', 'xt-tec-meetup-import' ) ?></p>
	    </div>
	    <?php
	}
	...
	public function xtmi_display_delete_success() {
	    ?>
	    <div class="notice notice-success is-dismissible">
	        <p><?php _e( 'Meetup URL deleted successfully.', 'xt-tec-meetup-import' ) ?></p>
	    </div>
	    <?php
	}
	...
	public function xtmi_display_setting_success() {
	    ?>
	    <div class="notice notice-success is-dismissible">
	        <p><?php _e( 'Meetup import settings saved successfully.', 'xt-tec-meetup-import' ) ?></p>
	    </div>
	    <?php
	}
